...
\newline
\newline
Next, the generator is trained to ``fool" the discriminator. 

\begin{comment}

 Typically, both the generator and the discriminator are neural networks.
Unlike FVBNs and VAEs, GANs only provide us with an implicit representation of $\hat{p}$. Unlike FVBNs, GANs can generate an entire sample simultaneously. Whereas VAEs require you to carefully select the distribution for the approximator $\hat{p}$, the neural networks used in GANs are generally considered universal approximators. Therefore, unlike (most) VAEs, a GAN trained with sufficient data can theoretically recover the true distribution $p$. 
