...
parser.add_argument('--dim_class_embedding', type=int, default=200, help='dimension of class embedding vector for discrimininator')
...
class _netG(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(nz+opt.dim_class_embedding, ngf * 8, 4, 1, 0, bias=False),
    ...
    def forward(self, noise_input, class_conditionals, batch_size):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            output = nn.parallel.data_parallel(self.main, input, range(self.ngpu))
        else:
            #print(noise_input)
            #print(class_conditionals)
            #embed = self.embedding(class_conditionals).view(batch_size,opt.dim_class_embedding,1,1)
            embed = self.embedding(class_conditionals)
            #print(embed)

            output = self.main(torch.cat([self.embedding(class_conditionals),noise_input],1).view(batch_size,nz+opt.dim_class_embedding,1,1))
            #sys.exit()
            '''
            print (self.embedding(class_conditionals).view(batch_size,opt.dim_class_embedding,1,1))
            print (noise_input)
            print (torch.cat([self.embedding(class_conditionals),noise_input],1))
            print (self.main(torch.cat([self.embedding(class_conditionals),noise_input],1)))
            sys.exit()
            '''
        return output
...
class _netD(nn.Module):
    ...
    def forward(self,image_input,conditional_input,batch_size):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            ...
            #get MLP(conditional_vector)
            conditional_output = self.mlp(conditional_input)

            #concatenate MLP(conditional_vector) with convolved feature map and pass through a final MLP
            final_output = self.mlp_final(torch.cat([image_output.view(batch_size,opt.dim_convolve_feature_map),conditional_output],1))
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        fake = netG(noise,noise_conditionals,batch_size)
    ...
    #fake = netG(fixed_noise_with_conditionals,50)
    fake = netG(fixed_noise,one_hots,50)
