'''Given the same noise vector z, condition on 10 different classes and visualize the output.'''
...
parser.add_argument('--netG_conditional', default='', help="path to netG (to continue training)")
...
#We will generate 11 images from one noise vector: 1 from vanilla model, 10 from conditional model (one image per class)
fixed_noise = torch.FloatTensor(1, nz).normal_(0, 1) #used for generating images
one_hot_labels = torch.LongTensor([i for i in range(10)]).resize_(10,1) 
one_hots = torch.zeros(10,opt.n_classes).scatter_(1, one_hot_labels, 1) 
fixed_noise_with_conditionals = torch.cat([fixed_noise,one_hots],1) '''THIS WONT WORK'''
...
'''SAVE ALL IMAGES'''
#vutils.save_image(fake.data, '%s/fake_samples.png' % (opt.outf, epoch), normalize=True)
